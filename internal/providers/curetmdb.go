...
// GetUpstreamTMDbClient 获取上游 TMDb 客户端的单例实例。
...
// NewCureTMDb 创建一个新的 CureTMDb 实例。
func NewCureTMDb() *CureTMDb {
	source := config.AppSettings.CURE_SOURCE
	// 使用配置的数据目录和文件名
	localPath := filepath.Join(config.AppSettings.DATA_DIR, "curetmdb.json")
...
// GetSeasonInfo 根据给定的 TMDB ID 获取季度信息。
...
// loadData 加载 CureTMDb 数据，优先从本地文件，然后从缓存。
...
// startPeriodicUpdate 启动一个 goroutine，定期从 sourceURL 下载数据并更新本地文件。
...
// updateLocalDataFromURL 从 sourceURL 下载数据并保存到本地文件。
